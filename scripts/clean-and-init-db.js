...
async function cleanAndInitializeDatabase() {
  ...
  try {
    ...
    // Drop and recreate schema (this will remove all tables and data)
    await sql`DROP SCHEMA IF EXISTS public CASCADE`;
    await sql`CREATE SCHEMA public`;
    console.log("✅ Schema recreated - all existing data removed");
    ...
    await sql`CREATE TABLE whatsapp_accounts (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
    ...
    await sql`CREATE TABLE whatsapp_webhooks (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    await sql`CREATE TABLE contact_groups (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
      description TEXT,
    ...
    await sql`CREATE TABLE contact_group_members (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    await sql`CREATE TABLE broadcasts (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
      description TEXT,
    ...
    await sql`CREATE TABLE automation_flows (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
      description TEXT,
    ...
    await sql`CREATE TABLE automation_executions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    await sql`CREATE TABLE segment_members (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    await sql`CREATE TABLE integrations (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
      type TEXT NOT NULL,
    ...
    await sql`CREATE TABLE webhook_logs (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    await sql`CREATE TABLE analytics_events (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      event TEXT NOT NULL,
      entity_type TEXT,
    ...
    await sql`CREATE TABLE analytics_metrics (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    await sql`CREATE TABLE ai_models (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      name TEXT NOT NULL,
      type TEXT NOT NULL,
    ...
    await sql`CREATE TABLE ai_predictions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    await sql`CREATE TABLE system_logs (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    // Application Settings & Configuration (CRITICAL FOR PERSISTENCE)
    await sql`CREATE TABLE application_settings (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
      key TEXT NOT NULL,
    ...
    await sql`CREATE TABLE user_preferences (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    // Media & File Management (FOR CDN INTEGRATION)
    await sql`CREATE TABLE media_files (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    // Enhanced Message Tracking (FOR COMPLETE ANALYTICS)
    await sql`CREATE TABLE message_tracking (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      organization_id UUID REFERENCES organizations(id) NOT NULL,
    ...
    // Chat Session Management (FOR BETTER REPORTING)
    ...
    await sql`CREATE INDEX idx_messages_contact_id ON messages(contact_id)`;
    await sql`CREATE INDEX idx_messages_created_at ON messages(created_at)`;
    await sql`CREATE INDEX idx_conversations_contact_id ON conversations(contact_id)`;
    await sql`CREATE INDEX idx_conversations_status ON conversations(status)`;
    await sql`CREATE INDEX idx_contacts_organization_id ON contacts(organization_id)`;
    await sql`CREATE INDEX idx_contacts_phone ON contacts(phone)`;
    await sql`CREATE INDEX idx_application_settings_org_category ON application_settings(organization_id, category)`;
    await sql`CREATE INDEX idx_analytics_events_timestamp ON analytics_events(timestamp)`;
    await sql`CREATE INDEX idx_webhook_logs_created_at ON webhook_logs(created_at)`;
